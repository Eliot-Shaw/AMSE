...
class _MyHomePageState extends State<MyHomePage> {
    ...
    Timer.periodic(const Duration(milliseconds: 1), (timer) {
      {
        ...
        setState(() {
          tempsEcouleString =
              "${(tempsEcoule / 1000000) ~/ 3600}:${((tempsEcoule / 1000000) ~/ 60) % 60}:${(tempsEcoule / 1000000).round() % 60}";
        });
      }
    });
    regenerateTiles();
          ...
          Row(
            children: [
              const Padding(
                ...
                child: Image(
                  height: 30,
                  image: AssetImage('assets/quality/dices.png'),
                ),
              ),
              Expanded(
                child: Slider(
  ...
  bool partieGagnee() {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              PageGagnante(pasWin, tempsEcouleStringWin, imageURLWin)),
    );
    return true;
  }
...
class PageGagnante extends StatelessWidget {
  ...
  const PageGagnante(this.pas, this.tempsEcouleStringWin, this.imageURL,
      {super.key});
